...
def make_rbm3(G, args):
    if os.path.isdir(args.rbm3_dirpath):
        ...
        n_gibbs_steps = np.arange(args.n_gibbs_steps[2],
                                  args.n_gibbs_steps[2] + epochs / n_every)
        learning_rate = args.lr[2] / np.arange(1, 1 + epochs / n_every)
        n_gibbs_steps = np.repeat(n_gibbs_steps, n_every)
        learning_rate = np.repeat(learning_rate, n_every)

        rbm3 = BernoulliRBM(n_visible=args.n_hiddens[1],
                            n_hidden=args.n_hiddens[2],
                            W_init=0.005,
                            vb_init=0.,
                            hb_init=0.,
                            n_gibbs_steps=n_gibbs_steps,
                            learning_rate=learning_rate,
                            momentum=[0.5] * 5 + [0.9],
                            max_epoch=max(args.epochs[2], n_every),
            ...
            dtype='float32',
            tf_saver_params=dict(max_to_keep=1),
            model_path=args.rbm3_dirpath)
        rbm3.fit(G)
    return rbm3


def make_dbm((X_train, X_val), rbms, (Q, G, K), args):
    if os.path.isdir(args.dbm_dirpath):
        ...
        dbm = DBM(rbms=rbms,
                  ...
                  h_particles_init=(Q[:args.n_particles].copy(),
                                    ...
                                    K[:args.n_particles].copy()),
                  n_gibbs_steps=args.n_gibbs_steps[3],
                  max_mf_updates=args.max_mf_updates,
                  mf_tol=args.mf_tol,
                  learning_rate=np.geomspace(args.lr[3], 5e-6, 400),
...
def make_mlp((X_train, y_train), (X_val, y_val),
             (W, hb), (W2, hb2), args):
    dense_params = {}
    ...
    model.compile(optimizer=MultiAdam(lr=0.001,
    ...
    with Stopwatch(verbose=True) as s:
        early_stopping = EarlyStopping(
            monitor=args.mlp_val_metric, patience=12, verbose=2)
    ...
    W1_finetuned, _ = model.layers[0].get_weights()
    W2_finetuned, _ = model.layers[2].get_weights()
...
def main():
    ...
    parser.add_argument('--n-hiddens', type=int, default=(1600, 1600, 1600), metavar='N', nargs='+',
                        help='numbers of hidden units')
    parser.add_argument('--n-gibbs-steps', type=int, default=(1, 1, 1, 1), metavar='N', nargs='+',
    ...
    parser.add_argument('--random-seed', type=int, default=(1337, 1111, 1111, 2222), metavar='N', nargs='+',
    ...
    parser.add_argument('--rbm3-dirpath', type=str, default='../models/myrbm3/', metavar='DIRPATH',
    ...
    X = np.delete(X, np.random.choice(X.shape[0], 1), axis=0)
    assert X.shape[0] % args.batch_size[3] == 0
    # Shuffle dataset
    ...
    # pre-train RBM #3
    rbm3 = make_rbm3(G, args)

    # freeze RBM #3 and extract features K = p_{RBM_3}(h|v=G)
    K = None
    if not os.path.isdir(args.dbm_dirpath):
        print "\nExtracting features from RBM #3 ..."
        K = rbm3.transform(G)
        print "\n"

    # jointly train DBM
    dbm = make_dbm((X_train, X_val), (rbm1, rbm2, rbm3), (Q, G, K), args)
    ...
    # make_mlp((X_train, y_train), (X_val, y_val),
    #          (W, hb), (W2, hb2), args)
